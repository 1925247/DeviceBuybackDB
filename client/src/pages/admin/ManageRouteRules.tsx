...
const ManageRouteRules: React.FC = () => {
  ...
  const renderDeleteModal = () => (
    <Dialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
      <DialogContent>
        ...
        <div className="py-4">
          <p className="font-semibold">Path: {selectedRoute?.path}</p>
          {selectedRoute?.description && <p>Description: {selectedRoute.description}</p>}
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={() => setIsDeleteModalOpen(false)}>
            Cancel
          </Button>
          <Button variant="destructive" onClick={handleDeleteRoute} disabled={deleteRouteMutation.isPending}>
  ...
  return (
    <AdminLayout>
      <div className="py-8 px-4">
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle>What are Route Rules?</CardTitle>
            <CardDescription>
              Route rules define how incoming requests are matched and directed to specific partners and regions.
              They are especially useful for PIN code-based lead assignment.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="text-sm space-y-4">
              <div>
                <h4 className="font-semibold">Path Patterns</h4>
                <p>
                  Routes match URL patterns. You can use placeholders like <code>:param</code> to match any value in that position.
                </p>
              </div>
              <div>
                <h4 className="font-semibold">PIN Codes</h4>
                <p>
                  PIN codes can be used to associate a partner with a specific route. When a customer enters a PIN code,
                  their request will be routed to the appropriate partner.
                </p>
              </div>
              <div>
                <h4 className="font-semibold">Priority</h4>
                <p>
                  Routes are evaluated in order of priority (highest first). More specific routes should have higher priority.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Table>
          <TableCaption>List of configured route rules.</TableCaption>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {routes.map((route) => (
              <TableRow key={route.id} className={!route.is_active ? 'opacity-50' : ''}>
                <TableCell className="font-medium">{route.path}</TableCell>
                <TableCell>{route.pin_code || 'N/A'}</TableCell>
...
export default ManageRouteRules;