...
import { Brand, brands as initialBrands } from '/home/project/src/db/brands';
const AdminBrands = () => {
  ...
  // Filter brands based on search term, active filter, and device type filter
  const filteredBrands = brands.filter((brand) => {
    const matchesSearch =
      brand.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      brand.slug.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesActiveFilter = filterActive === null || brand.active === filterActive;
    const matchesDeviceTypeFilter =
      filterDeviceType === '' || brand.deviceTypes.includes(filterDeviceType);
    return matchesSearch && matchesActiveFilter && matchesDeviceTypeFilter;
  });
  ...
  const handleInputChange = (
    ...
    if (name === 'deviceTypes') {
      // Handle multi-select for device types
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-700">
            Manage the brands of devices that can be sold on your platform
          </p>
        </div>
                      ...
                      <td className="px-3 py-4 text-sm text-gray-500">
                        <div className="flex flex-wrap gap-1">
                          {brand.deviceTypes.map((type) => (
                            ...
                            >
                              {deviceTypeOptions.find((option) => option.value === type)?.label || type}
                            </span>
                          ))}
                        </div>
                      </td>
                          ...
                          {formData.logo && (
                            <div className="mt-2 flex justify-center">
                              <img
                                src={formData.logo}
                                alt="Logo preview"
                                className="h-16 w-16 object-contain bg-gray-100 rounded p-1"
                              />
                            </div>
                          )}
                        </div>
                        <div>
                          <label htmlFor="deviceTypes" className="block text-sm font-medium text-gray-700">
                            Device Types
                          </label>
                          <select
                            id="deviceTypes"
