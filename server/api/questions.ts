      ...
      const answerPromises = validatedData.answerChoices.map(async (choice) => {
        const choiceQuery = `
          INSERT INTO answer_choices (
            question_id, text, value, "order", impact_multiplier, created_at, updated_at
          )
          VALUES ($1, $2, $3, $4, $5, NOW(), NOW())
          RETURNING id, question_id as "questionId", text, value, "order", 
                    impact_multiplier as "impactMultiplier", 
      ...
      const answerPromises = validatedData.answerChoices.map(async (choice) => {
        if (choice.id && existingChoiceIds.has(choice.id)) {
          ...
          const choiceQuery = `
            ...
            SET text = $1, value = $2, "order" = $3, impact_multiplier = $4, updated_at = NOW()
