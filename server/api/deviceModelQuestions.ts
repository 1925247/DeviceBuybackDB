 ...
 */
export async function getDeviceModelQuestions(req: Request, res: Response) {
  try {
 ...
 */
export async function mapQuestionsToDeviceModel(req: Request, res: Response) {
  try {
    const { modelId } = req.params;
    ...
    if (questionIds.length > 0) {
      const mappings = questionIds.map((questionId: number) => ({
        modelId: Number(modelId),
 ...
 */
export async function getAvailableQuestions(req: Request, res: Response) {
  try {
 ...
 */
export async function getQuestionMappingStats(req: Request, res: Response) {
  try {
